    ...
    public sealed partial class MergePDF : Page, IDisposable
    {
        ...
        private void MergePDF_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (firstDocumentStream == null)
                firstDocumentStream = typeof(MergePDF).GetTypeInfo().Assembly.GetManifestResourceStream(path + "Assets.pdf_succinctly.pdf");
            //Get template PDF file stream from assembly.
            if (secondDocumentStream == null)
                secondDocumentStream = typeof(MergePDF).GetTypeInfo().Assembly.GetManifestResourceStream(path + "Assets.form_template.pdf");
